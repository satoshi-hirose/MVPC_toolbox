function save_figures_compare_teROIs(MY_VAR)
...
for methodn = 1:size(Method_info,1)
    for sp = 1:size(Spatial_ROI,1)
        ...
        % get percent correct
        test_correct_mean = NaN(size(Percent_correct{sp,1},2),size(Temporal_ROI,1));
        teROI_names = cell(1,size(Temporal_ROI,1));
        for te = 1:size(Temporal_ROI,1)
            teROI_names{te} = num2str(Temporal_ROI{te}.timing);
            for sub = 1:size(Percent_correct{sp,te},2)
                test_correct_mean(sub,te) = Percent_correct{sp,te}{methodn,sub};
            end
        end

       %% percent correct bar graph
       hold off     
       bar(mean(test_correct_mean,1),'k'); hold on
            plot(test_correct_mean','ko--','LineWidth',1,'MarkerFaceColor','w')
            
            plot([0 size(test_correct_mean,2)+1],[50 50],'k--');plot([0 size(test_correct_mean,2)+1],[75 75],'k--');plot([0 size(test_correct_mean,2)+1],[25 25],'k--');hold off;
            axis([0.5 size(test_correct_mean,2)+0.5 0 100]); % set y -axis 0 to 100
            
            set(gca,'xTick',1:size(test_correct_mean,2),'xTickLabel',teROI_names)
        
        %% title setting
        title_str = strvcat('PERFORMANCE COMPARISON ACROSS TEMPORAL ROIS', ['Spatial ROI=' Spatial_ROI{sp}.ROI_param.method,'   Decoding Algorithm=' Method_info{methodn}.name]);
